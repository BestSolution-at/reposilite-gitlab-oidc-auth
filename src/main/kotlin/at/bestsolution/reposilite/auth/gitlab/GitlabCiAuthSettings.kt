...
data class GitlabCiAuthSettings(
    ...
    @get:Doc(title = "CI Read Repositories", description = "Repositories accessible with READ for all CI builds")
    val ciReadRepositories: List<String> = emptyList(),
    @get:Doc(title = "CI Write Repositories", description = "Repositories accessible with WRITE for protected branch builds")
    val ciWriteRepositories: List<String> = emptyList(),
) : SharedSettings
